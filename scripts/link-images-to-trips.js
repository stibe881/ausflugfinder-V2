...
async function linkImagesToTrips() {
    ...
    try {
        ...
        for (const exc of data.excursions) {
            try {
                // Find trip by name
                const [trips] = await connection.execute("SELECT id FROM trips WHERE title = ?", [exc.name]);
                ...
                if (exc.title_image) {
                    ...
                    try {
                        await connection.execute("INSERT INTO tripPhotos (tripId, photoUrl, caption, isPrimary) VALUES (?, ?, ?, ?)", [tripId, imageUrl, `${exc.name} - Cover Image`, 1]);
